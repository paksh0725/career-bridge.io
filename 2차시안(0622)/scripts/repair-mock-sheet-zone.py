#!/usr/bin/env python3
"""Repair broken sheet-zone wrap and apply correct structure."""
...
def wrap_mock_tail(html: str) -> str:
    html = ORPHAN.sub('\n', html)
    pos = 0
    while True:
        ...
        s0, s1 = stack
        inner = html[s0:s1]
        if 'wt-mock-sheet-stack' in inner:
            pos = idx + 1
            continue
...
def move_orphans(html: str) -> str:
    ...
    while True:
        ...
        sc_idx = zone_html.find('wt-sheet-scroll')
        if sc_idx < 0:
            pos = idx + 1
            continue
        sc_open = zone_html.rfind('<div', 0, sc_idx)
...
def main():
    ...
    for html_path in sorted(ROOT.rglob('*.html')):
        if any(p in html_path.parts for p in ('student', 'scripts', 'Test')):
            continue
        text = html_path.read_text(encoding='utf-8')
        if 'wt-stack-layout' not in text or 'wt-mock-block' not in text:
            continue
        new_text = wrap_mock_tail(text)
        new_text = move_orphans(new_text)
        if new_text != text:
            html_path.write_text(new_text, encoding='utf-8')
            print('fixed', html_path.relative_to(ROOT))
            count += 1
    print('done', count)
